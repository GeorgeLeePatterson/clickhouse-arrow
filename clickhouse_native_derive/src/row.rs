...
fn build_generics(cont: &Container) -> syn::Generics {
    ...
    match cont.attrs.bound() {
        ...
        None => bound::with_bound(cont, &generics, needs_serialize_bound, &[
            &parse_quote!(::clickhouse_native::FromSql),
            &parse_quote!(::clickhouse_native::ToSql),
        ]),
    }
}
...
pub fn expand_derive_serialize(
    ...
    let schema_body = Stmts(schema_body(&cont, &params));
    ...
    let const_column_count_fn = format_ident!("__{ident}_column_count_clickhouse_native");
    ...
    let impl_block = quote! {
        ...
        use ::clickhouse_native::{ToSql as _, FromSql as _};
        #[automatically_derived]
        #[allow(clippy)]
        #[allow(non_snake_case)]
        #[allow(clippy::absurd_extreme_comparisons)]
        impl #impl_generics ::clickhouse_native::Row for #ident #ty_generics #where_clause {
            ...
            fn to_schema() -> Option<Vec<(String, ::clickhouse_native::Type, Option<::clickhouse_native::Value>)>> {
                #schema_body
            }

            fn deserialize_row(map: Vec<(&str, &::clickhouse_native::Type, ::clickhouse_native::Value)>) -> ::clickhouse_native::Result<Self> {
                #deserialize_body
            }

            fn serialize_row(self, type_hints: &[(String, ::clickhouse_native::Type)]) -> ::clickhouse_native::Result<Vec<(::std::borrow::Cow<'static, str>, ::clickhouse_native::Value)>> {
                #serialize_body
            }
        }
    };
...
fn schema_body(cont: &Container, _params: &Parameters) -> Fragment {
    if let Some(schema_path) = cont.attrs.schema() {
        // If schema attribute is provided, call the specified function
...
fn serialize_length_body(cont: &Container, _params: &Parameters) -> Fragment {
    if let Some(_type_into) = cont.attrs.type_into() {
            ...
            .filter(|&field| !field.attrs.skip_serializing() && field.attrs.nested())
        {
            ...
            total = quote! { match <#field_ty as ::clickhouse_native::Row>::COLUMN_COUNT { Some(x) => (#total) + x, None => return None, } };
        }
        ...
        let name_sources = cont.data.iter().filter(|&field| !field.attrs.skip_serializing())
            .map(|field| {
                ...
                if field.attrs.nested() {
                    let field_ty = unwrap_vec_type(field.ty).expect("invalid non-Vec nested type");
                    quote! { out.extend(<#field_ty as ::clickhouse_native::Row>::column_names()?.into_iter().map(|x| ::std::borrow::Cow::Owned(format!("{}.{}", #name, x)))); }
...
fn serialize_into(params: &Parameters, type_into: &syn::Type) -> Fragment {
    ...
    quote_block! {
        ::clickhouse_native::Row::serialize_row(
            ...
            match field.attrs.serialize_with() {
                ...
                None => {
                    if field.attrs.nested() {
                        ...
                        quote! {
                            {
                                let inner_length = <#field_ty as ::clickhouse_native::Row>::COLUMN_COUNT.expect("nested structure must have known length");
                                let mut outputs: ::std::vec::Vec<(::std::option::Option<::std::borrow::Cow<str>>, ::std::vec::Vec<::clickhouse_native::Value>)> = ::std::vec::Vec::with_capacity(inner_length);
                                for _ in 0..inner_length {
                                    outputs.push((None, ::std::vec::Vec::new()));
                                }
                                for row in #field_expr.into_iter() {
                                    let columns = <#field_ty as ::clickhouse_native::Row>::serialize_row(row, type_hints)?;
                                    ...
                                    let name = match name {
                                        ...
                                        None => {
                                            if column_names.is_none() {
                                                column_names = Some(<#field_ty as ::clickhouse_native::Row>::column_names().expect("column_names required for empty nested serialization"));
                                            }
                                            format!("{}.{}", #key_expr, column_names.as_ref().unwrap().get(i).expect("missing column_name for nested struct")).into()
                                        }
                                    };
                                    out.push((name, ::clickhouse_native::Value::Array(values)));
                                }
                            }
                        }
                    } else if field.attrs.flatten() {
                        ...
                        quote! {
                            let type_hint = type_hints.iter().find(|(name, _)| name == #key_expr).map(|(_, hint)| hint);
                            out.push((::std::borrow::Cow::Borrowed(#key_expr), <#field_ty as ::clickhouse_native::ToSql>::to_sql(#field_expr, type_hint)?));
                        }
                    }
                },
            }
...
fn deserialize_from(type_from: &syn::Type) -> Fragment {
    quote_block! {
        ::clickhouse_native::Result::map(
            <#type_from as ::clickhouse_native::Row>::deserialize_row(map),
...
fn deserialize_try_from(type_try_from: &syn::Type) -> Fragment {
    quote_block! {
        ::clickhouse_native::Result::and_then(
            <#type_try_from as ::clickhouse_native::Row>::deserialize_row(map),
...
fn deserialize_map(
        ...
        .for_each(|(field, name)| {
            ...
            if field.attrs.nested() {
                ...
                nested_temp_decls.push(quote_spanned! { span=>
                    let #size_field = <#field_ty as ::clickhouse_native::Row>::COLUMN_COUNT.expect("nested structure must have known length");
                    let mut #deser_name_ext: Vec<(&str, &::clickhouse_native::Type)> = Vec::with_capacity(#size_field);
                    let mut #deser_name_ext_iter: Vec<::std::vec::IntoIter<::clickhouse_native::Value>> = Vec::with_capacity(#size_field);
                ...
                nested_rectify.push(quote_spanned! { span=>
                    {
                        ...
                        'outer: loop {
                            ...
                            for (name, type_) in #deser_name_ext.iter() {
                                temp.push((*name, *type_, ::clickhouse_native::Value::Null));
                            }
                            ...
                            #name.as_mut().unwrap().push(<#field_ty as ::clickhouse_native::Row>::deserialize_row(temp)?);
                        }
                    }
                });
                return;
            ...
            let visit = match field.attrs.deserialize_with() {
                None => {
                    let field_ty = field.ty;
                    let span = field.original.span();
                    quote_spanned!(span=> <#field_ty as ::clickhouse_native::FromSql>::from_sql(_type_, _value).map_err(|e| e.with_column_name(#deser_name))?)
                }
                Some(path) => {
                    let span = field.original.span();
    ...
    let mut pull_flatten: Vec<TokenStream> = vec![quote! {
        let mut map = map;
        let mut map_flattened_fields = std::collections::HashMap::<&str, (&::clickhouse_native::Type, ::clickhouse_native::Value)>::default();
    }];
    ...
    let result = fields_names.iter().map(|(field, name)| {
        ...
        if field.attrs.skip_deserializing() {
            ...
            quote! {
                ...
                clickhouse_native::Row::deserialize_row(map2)? }
            }
        } else {
            quote!(#member: #name)
        }
    ...
    quote_block! {
        ...
        ::clickhouse_native::Result::Ok(#result)
    }
}
