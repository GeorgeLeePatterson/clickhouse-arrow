// This module handles parsing of `#[clickhouse_native(...)]` attributes. The entrypoints
    ...
    fn set<A: ToTokens>(&mut self, obj: A, value: T) {
        ...
        if self.value.is_some() {
            self.cx.error_spanned_by(
                tokens,
                format!("duplicate clickhouse_native attribute `{}`", self.name),
            );
        } else {
            self.tokens = tokens;
...
pub struct Container {
    ...
    // #[clickhouse_native(schema = "get_schema")]
...
impl Container {
    pub fn from_ast(cx: &Ctxt, item: &syn::DeriveInput) -> Self {
        ...
        for attr in &item.attrs {
            if attr.path().is_ident("clickhouse_native") {
                let _ = attr.parse_nested_meta(|meta| {
                    match meta.path.get_ident() {
                        Some(ident) if ident == RENAME => {
                            if let Ok(expr) = meta.value() {
                                if let Ok(s) = expr.parse::<syn::LitStr>() {
                        ...
                        Some(ident) if ident == DEFAULT => {
                            if meta.input.peek(syn::Token![=]) {
                                if let Ok(expr) = meta.value() {
                                    if let Ok(path) = expr.parse::<syn::ExprPath>() {
                                        match &item.data {
                                            syn::Data::Struct(syn::DataStruct {
                                                ...
                                                _ => cx.error_spanned_by(
                                                    fields,
                                                    "#[clickhouse_native(default = \"...\")] can \
                                                     only be used on structs with named fields",
                                                ),
                                            },
                                            _ => cx.error_spanned_by(
                                                item,
                                                "#[clickhouse_native(default = \"...\")] can only \
                                ...
                                match &item.data {
                                    syn::Data::Struct(syn::DataStruct { fields, .. }) => {
                                        match fields {
                                            ...
                                            _ => cx.error_spanned_by(
                                                fields,
                                                "#[clickhouse_native(default)] can only be used \
                                    ...
                                    _ => cx.error_spanned_by(
                                        item,
                                        "#[clickhouse_native(default)] can only be used on \
                        ...
                        Some(ident) if ident == FROM => {
                            if let Ok(expr) = meta.value() {
                                if let Ok(ty) = expr.parse::<syn::Type>() {
                            ...
                            if let Ok(expr) = meta.value() {
                                if let Ok(path) = expr.parse::<syn::ExprPath>() {
                                    ...
                                    cx.error_spanned_by(
                                        meta.path,
                                        format!(
                                            "expected function path for `schema` attribute, got: \
                                             {:?}",
                                            expr
                                        ),
                                    );
                        ...
                        _ => {
                            ...
                            cx.error_spanned_by(
                                meta.path,
                                format!("unknown clickhouse_native container attribute `{}`", path),
                            );
                        }
                    }
                    Ok(())
                ...
                let _ = attr.parse_nested_meta(|meta| {
                    match meta.path.get_ident() {
                        ...
                        _ => {
                            ...
                            cx.error_spanned_by(
                                meta.path,
                                format!("unknown clickhouse_native field attribute `{}`", path),
                            );
                        }
                    }
                    Ok(())
...
pub fn get_clickhouse_native_meta_items(cx: &Ctxt, attr: &syn::Attribute) -> Result<(), ()> {
    ...
    attr.parse_nested_meta(|meta| {
        cx.error_spanned_by(meta.path, "unexpected literal in clickhouse_native attribute");
        Ok(())
    })
    .map_err(|err| {
        cx.syn_error(err);
...
fn get_lit_str(cx: &Ctxt, attr_name: Symbol, expr: &syn::Expr) -> Result<syn::LitStr, ()> {
    if let syn::Expr::Lit(syn::ExprLit { lit: syn::Lit::Str(lit), .. }) = expr {
        ...
        cx.error_spanned_by(
            expr,
            format!(
                "expected clickhouse_native {} attribute to be a string: `{} = \"...\"`",
                attr_name, attr_name
            ),
        );
        Err(())
