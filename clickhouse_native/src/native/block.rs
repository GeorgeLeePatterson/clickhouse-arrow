...
impl ProtocolData<Self> for Block {
    ...
    async fn read<R: ClickhouseRead>(reader: &mut R, revision: u64, _options: ()) -> Result<Self> {
        let info = if revision > 0 { BlockInfo::read(reader).await? } else { BlockInfo::default() };
