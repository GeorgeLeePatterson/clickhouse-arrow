...
///     serialize(&Type::Enum8(pairs), &field, &array, &mut writer, &mut state)
///         .await
///         .unwrap();
...
mod tests {
    ...
    use arrow::datatypes::{Field, Int8Type, Int16Type};
    ...
    async fn test_serialize_enum8_dictionary() {
        ...
        let mut state = SerializerState::default();
        serialize(&Type::Enum8(pairs), &field, &array, &mut writer, &mut state).await.unwrap();
    ...
    async fn test_serialize_enum16_dictionary() {
        ...
        let mut writer = MockWriter::new();
        let mut state = SerializerState::default();
        serialize(&Type::Enum16(pairs), &field, &array, &mut writer, &mut state).await.unwrap();
    ...
    async fn test_serialize_enum16_uint_type_ok() {
        ...
        let mut writer = MockWriter::new();
        let mut state = SerializerState::default();
        let result = serialize(&Type::Enum16(pairs), &field, &array, &mut writer, &mut state).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_write_enum8_string_like_values() {
        let cases = vec![
            ...
            (
                Field::new("", DataType::Utf8View, true),
                Arc::new(StringViewArray::from(vec![Some("a"), Some("b"), None])) as ArrayRef,
            ),
            (
                Field::new("", DataType::LargeUtf8, true),
            ...
            (
                Field::new("", DataType::BinaryView, true),
                Arc::new(BinaryViewArray::from(vec![Some(b"a" as &[u8]), Some(b"b"), None]))
                    as ArrayRef,
            ),
            (
                Field::new("", DataType::LargeBinary, true),
        ...
        for (field, array) in cases {
            let mut writer = MockWriter::new();
            let mut state = SerializerState::default();
            serialize(&Type::Enum8(enum_values.clone()), &field, &array, &mut writer, &mut state)
                .await
                .unwrap();
