...
mod tests {
    ...
    async fn test_serialize_string() {
        ...
        serialize(&Type::String, &column, &mut writer).await.unwrap();
        let expected = vec![
    ...
    async fn test_serialize_binary() {
        let column =
            Arc::new(BinaryArray::from(vec![Some(b"abc".as_ref()), None, Some(b"def".as_ref())]))
                as ArrayRef;
        let mut writer = MockWriter::new();
        serialize(&Type::Binary, &column, &mut writer).await.unwrap();
        let expected = vec![
    ...
    async fn test_serialize_fixed_string() {
        ...
        let mut writer = MockWriter::new();
        serialize(&Type::FixedSizedString(5), &column, &mut writer).await.unwrap();
        let expected = vec![
    ...
    async fn test_serialize_fixed_string_short_and_null() {
        ...
        let mut writer = MockWriter::new();
        serialize(&Type::FixedSizedString(3), &column, &mut writer).await.unwrap();
        let expected = vec![
            97, 0, 0, // "a" + padding
    ...
    async fn test_serialize_fixed_string_oversized() {
        ...
        let mut writer = MockWriter::new();
        let result = serialize(&Type::FixedSizedString(3), &column, &mut writer).await;
    ...
    async fn test_serialize_fixed_binary() {
        ...
        let mut writer = MockWriter::new();
        serialize(&Type::FixedSizedBinary(5), &column, &mut writer).await.unwrap();
        let expected = vec![
            97, 98, 99, 0, 0, // "abc" + padding
    ...
    async fn test_serialize_fixed_binary_null() {
        ...
        let mut writer = MockWriter::new();
        serialize(&Type::FixedSizedBinary(3), &column, &mut writer).await.unwrap();
        let expected = vec![
            97, 98, 0, // "ab" + padding
            0, 0, 0, // null (all zeros)
            99, 100, 0, // "cd" + padding
    ...
    async fn test_serialize_fixed_binary_oversized() {
        ...
        let mut writer = MockWriter::new();
        let result = serialize(&Type::FixedSizedBinary(3), &column, &mut writer).await;
        assert!(result.is_ok(), "Expected truncated string");
    }
