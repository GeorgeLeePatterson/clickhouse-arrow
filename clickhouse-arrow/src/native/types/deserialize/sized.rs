    ...
    async fn read<R: ClickHouseRead>(
        ...
        for _ in 0..rows {
            out.push(match type_ {
                ...
                Type::Decimal32(s) => {
                    Value::Decimal32(usize::from(*s), reader.read_i32_le().await?)
                }
                Type::Decimal64(s) => {
                    Value::Decimal64(usize::from(*s), reader.read_i64_le().await?)
                }
                Type::Decimal128(s) => {
                    Value::Decimal128(usize::from(*s), reader.read_i128_le().await?)
                }
                Type::Decimal256(s) => {
                    let mut buf = [0u8; 32];
                    let _ = reader.read_exact(&mut buf[..]).await?;
                    buf.reverse();
                    Value::Decimal256(usize::from(*s), i256(buf))
                }
                ...
                Type::DateTime64(precision, tz) => {
                    ...
                    Value::DateTime64(DynDateTime64(*tz, raw, usize::from(*precision)))
                }
                Type::Enum8(pairs) => {
