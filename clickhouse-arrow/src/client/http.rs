...
use crate::constants::{CLICKHOUSE_DEFAULT_CHUNK_BYTES, TCP_KEEP_ALIVE_SECS};
use crate::prelude::*;
use crate::row::protocol::{
    DEFAULT_SETTINGS, HttpSummary, X_CLICKHOUSE_FORMAT, X_CLICKHOUSE_KEY, X_CLICKHOUSE_SUMMARY,
    X_CLICKHOUSE_USER,
};
...
// TODO: Remove - IMPORTANT! Put back to RECV_BUFFER_SIZE
const TARGET_BUFFER_SIZE: usize = CLICKHOUSE_DEFAULT_CHUNK_BYTES;
const MIN_COMPRESSED_TARGET_BUFFER_SIZE: usize = CLICKHOUSE_DEFAULT_CHUNK_BYTES; // 64 * 1024; // Allow serving bytes early
...
impl HttpClient {
    ...
    async fn send_query<P: Into<QueryParams>>(
        ...
        #[cfg(feature = "profile")]
        let guard = non_continuous_frame!("send_query");

        #[cfg(feature = "profile")]
        let span = tracy_span!("url_parse");
        ...
        #[cfg(feature = "profile")]
        let span = tracy_span!("query_params");
        ...
        #[cfg(feature = "profile")]
        let req_guard = non_continuous_frame!("send_query_request");
        ...
        let body = HyperBodyReader::try_new(response, self.options.metadata.compression)?;
...
impl HyperBodyReader {
    pub(crate) fn try_new(
        ...
        #[cfg(feature = "profile")]
        let _span = tracy_span!("hyper_body_try_new");
...
impl Stream for HyperBodyReader {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        #[cfg(feature = "profile")]
        let _span = tracy_span!("hyper_body_poll");
        ...
        let min_target = MIN_COMPRESSED_TARGET_BUFFER_SIZE;
        ...
        if !*this.is_finished {
            ...
            *this.is_finished = loop {
                #[cfg(feature = "profile")]
                let _frame_span = tracy_span!("hyper_body_poll_frame");
                ...
                if matches!(res, Poll::Pending) {
                    if this.buffer.len() >= min_target {
                        // TODO: Remove
                        #[cfg(feature = "profile")]
                        plot!("hyper_body_pending_exit_buf_len", this.buffer.len() as f64);
                ...
                match ready!(res) {
                    Some(Ok(frame)) => {
                        if let Ok(data) = frame.into_data() {
                            // TODO: Remove
                            #[cfg(feature = "profile")]
                            plot!("hyper_body_frame_buf_len", data.len() as f64);
        ...
        #[cfg(feature = "profile")]
        let _split_span = tracy_span!("hyper_body_buffer_split");
